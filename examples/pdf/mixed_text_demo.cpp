...
int main() {
    try {
        ...
        pdf.add_text("TinaKit 中英混合文本处理演示", {100, 50}, title_font);
        ...
        pdf.add_text("测试1: Hello 世界! Welcome to 中国!", {100, 100}, test_font);

        // 测试用例2：技术术语混合
        pdf.add_text("测试2: 使用 C++ 开发 PDF library 很有趣!", {100, 120}, test_font);

        // 测试用例3：数字和符号混合
        pdf.add_text("测试3: 价格 $99.99 人民币 ¥688.00 优惠 20%", {100, 140}, test_font);

        // 测试用例4：长句子混合
        pdf.add_text("测试4: TinaKit is a powerful C++20 library for processing OpenXML files like Excel (.xlsx), Word (.docx), and generating PDF documents. 它支持中英文混合文本的智能处理。", {100, 160}, test_font);

        // 测试用例5：纯英文（使用中文字体）
        pdf.add_text("Test 5: Pure English text using Chinese font (SimSun)", {100, 180}, test_font);

        // 测试用例6：纯中文
        pdf.add_text("测试6: 纯中文文本测试，应该正常显示", {100, 200}, test_font);
        ...
        pdf.add_text("字体回退测试: Using Helvetica font for 中英混合 text", {100, 240}, english_font);
        ...
        pdf.add_text("Times字体测试: Times-Roman font with 中文字符 mixed content", {100, 260}, times_font);
        ...
        pdf.add_text("特殊字符: 《TinaKit》—— 一个现代化的 C++ 库！", {100, 300}, special_font);
        pdf.add_text("标点符号: Hello, 世界! How are you? 你好吗？", {100, 320}, special_font);
        pdf.add_text("数学符号: α + β = γ, 面积 = π × r²", {100, 340}, special_font);
        ...
        pdf.add_text("技术说明:", {100, 380}, info_font);
        pdf.add_text("• 智能文本分段：自动识别中英文字符，分别处理", {120, 400}, info_font);
        pdf.add_text("• 字体回退机制：英文字体自动回退到中文字体显示CJK字符", {120, 420}, info_font);
        pdf.add_text("• Unicode支持：完整支持UTF-8编码和UTF-16BE转换", {120, 440}, info_font);
        pdf.add_text("• 渲染优化：ASCII字符使用括号格式，CJK字符使用十六进制格式", {120, 460}, info_font);
        ...
        pdf.add_text("Generated by TinaKit PDF Engine v1.0 - Mixed Text Processing Demo", {100, 500}, version_font);
